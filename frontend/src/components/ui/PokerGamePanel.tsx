...
export function PokerGamePanel({ showSitButton = true, compact = false }: PokerGamePanelProps) {
  ...
  const revealResults = (
    ...
    if (firstWin) {
      addMessage('🎁 首胜大礼包 · 高价值分享卡已解锁，快去保存海报！');
    }
  ...
  const startSolo = async () => {
    ...
    try {
      ...
      clearPokerRoom();
      revealResults(r.results, r.won, r.pot, r.net, r.balance, r.community_cards, r.tie, r.winners_count, tier.buyIn, r.highlight_broadcast, r.first_win);
    } catch {
      addMessage('发牌失败，请重试');
    } finally {
      setPhase('idle');
  ...
  const startMultiplayer = async () => {
    ...
    try {
      ...
      revealResults(
        ...
        (r as { first_win?: boolean }).first_win,
      );
    } finally {
      setPhase('idle');
      setPokerTableDealingUntil(0);
      ...
      {inRoom && (
        <div style={{ marginBottom: 10, padding: 10, background: '#eef4ff', borderRadius: 8, fontSize: 12, border: '1px solid #7aa8e8' }}>
          ...
          <div style={{ fontSize: 11, color: '#5a7a9a', marginBottom: 8 }}>
            复制链接邀请好友入座 · 也可手动输入 5 位房间号
          </div>
          <button type="button" className="ui-btn" style={{ width: '100%', marginBottom: 8, fontSize: 11 }}
            onClick={async () => {
              const code = pokerRoom.room_code || pokerRoom.id;
