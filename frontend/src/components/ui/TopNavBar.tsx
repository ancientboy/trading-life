import { useEffect, useState } from 'react';
import { WalletIcon, SparklesIcon } from '@heroicons/react/24/solid';
import {
  GiftIcon, Cog6ToothIcon, QuestionMarkCircleIcon, TrophyIcon, EllipsisHorizontalIcon,
} from '@heroicons/react/24/outline';
import {
  GiftIcon as GiftSolid, Cog6ToothIcon as CogSolid, QuestionMarkCircleIcon as HelpSolid,
  TrophyIcon as TrophySolid, EllipsisHorizontalIcon as EllipsisSolid,
} from '@heroicons/react/24/solid';
import { useGameStore } from '../../store/useGameStore';
import { AppIcon, NavIcon } from '../icons/AppIcon';
import { getStoredAccount, clearAuthSession } from '../../lib/lifeAuth';
import { authLogout } from '../../lib/lifeApi';
import { CoachMarks } from './CoachMarks';

export function TopNavBar() {
  const ticker = useGameStore(s => s.ticker);
  const overview = useGameStore(s => s.overview);
  const userPortfolio = useGameStore(s => s.userPortfolio);
  const points = useGameStore(s => s.points);
  const openWorkshop = useGameStore(s => s.openWorkshop);
  const dailyAllowanceClaimed = useGameStore(s => s.dailyAllowanceClaimed);
  const dailyAllowanceAmount = useGameStore(s => s.dailyAllowanceAmount);
  const claimDailyAllowance = useGameStore(s => s.claimDailyAllowance);
  const openModal = useGameStore(s => s.openModal);
  const [hover, setHover] = useState<string | null>(null);
  const [userMenuOpen, setUserMenuOpen] = useState(false);
  const [moreOpen, setMoreOpen] = useState(false);
  const [showCoach, setShowCoach] = useState(false);
  const [isMobile, setIsMobile] = useState(false);

  useEffect(() => {
    const mq = window.matchMedia('(max-width: 768px)');
    const apply = () => setIsMobile(mq.matches);
    apply();
    mq.addEventListener('change', apply);
    return () => mq.removeEventListener('change', apply);
  }, []);

  const account = getStoredAccount();
  const pnl = overview.total_pnl || 0;
  const capital = overview.total_capital || 0;
  const pnlPct = overview.total_pnl_pct ?? (capital ? (pnl / capital * 100) : 0);

  const logout = async () => {
    await authLogout().catch(() => {});
    clearAuthSession();
    window.location.reload();
  };

  const openHelp = () => {
    if (!localStorage.getItem('tl_coach_seen')) {
      setShowCoach(true);
      return;
    }
    openModal('help');
  };

  const closeMenus = () => {
    setUserMenuOpen(false);
    setMoreOpen(false);
  };

  return (
    <>
      <header className="top-nav">
        <div className="top-nav-brand">
          <div style={{ fontWeight: 700, fontSize: 17, letterSpacing: 0.3, display: 'flex', alignItems: 'center', gap: 6 }}>
            <span className="brand-mark" aria-hidden>🐧</span>
            <span style={{ color: '#3d3530' }}>交易人生</span>
          </div>
          {(overview.total_trades ?? 0) > 0 && (
            <div style={{ fontSize: 10, color: '#6b8e4e', marginTop: 2 }}>
              📈 模拟盘运行中 · {overview.total_trades} 笔成交
            </div>
          )}
        </div>

        <div className="top-nav-stats">
          <div className="stat-card">
            <div className="label" style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
              <AppIcon icon={WalletIcon} size="mini" color="gold" /> 模拟总资产
            </div>
            <div className="value mono gold">${Math.round(capital).toLocaleString()}</div>
            {userPortfolio && (
              <div style={{ fontSize: 9, color: '#9a8b7a', marginTop: 2 }}>现金 ${Math.round(userPortfolio.cash).toLocaleString()}</div>
            )}
          </div>
          <div className="stat-card">
            <div className="label">累计盈亏</div>
            <div className={`value mono ${pnl >= 0 ? 'profit' : 'loss'}`}>
              {pnl >= 0 ? '+' : ''}${Math.round(pnl).toLocaleString()}
            </div>
          </div>
          <div className="stat-card">
            <div className="label">收益率</div>
            <div className={`value mono ${pnlPct >= 0 ? 'profit' : 'loss'}`}>
              {pnlPct >= 0 ? '+' : ''}{pnlPct.toFixed(2)}%
            </div>
          </div>
          <div className="stat-card" style={{ minWidth: 100 }}>
            <div className="label" style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
              <AppIcon icon={SparklesIcon} size="mini" color="gold" /> 积分
            </div>
            <div className="value mono gold" style={{ fontSize: 14 }}>{points.toLocaleString()}</div>
            {!dailyAllowanceClaimed && (
              <button className="ui-btn" style={{ marginTop: 4, fontSize: 10, padding: '2px 6px', width: '100%' }}
                onClick={() => void claimDailyAllowance()}>
                领 {dailyAllowanceAmount}
              </button>
            )}
          </div>
          <div className="stat-card top-nav-btc-card" style={{ minWidth: 80 }}>
            <div className="label">BTC</div>
            <div className="value mono" style={{ fontSize: 13 }}>{ticker.BTCUSDT ? '$' + Math.round(ticker.BTCUSDT).toLocaleString() : '--'}</div>
            <button className="ui-btn" style={{ marginTop: 4, fontSize: 9, padding: '2px 4px', width: '100%' }}
              onClick={() => openWorkshop('create')} title="创建交易 Agent">
              + 模拟盘
            </button>
          </div>
        </div>

        <div className="top-nav-actions">
          <button className="ui-btn nav-icon-btn" onClick={() => { closeMenus(); openModal('shop'); }}
            onMouseEnter={() => setHover('shop')} onMouseLeave={() => setHover(null)} title="积分商城 · 含场景装扮">
            <NavIcon outline={GiftIcon} solid={GiftSolid} hovered={hover === 'shop'} size="nav" />
            <span className="nav-icon-label">商城</span>
          </button>
          <button className="ui-btn nav-icon-btn" onClick={() => { closeMenus(); openHelp(); }}
            onMouseEnter={() => setHover('help')} onMouseLeave={() => setHover(null)} title="帮助">
            <NavIcon outline={QuestionMarkCircleIcon} solid={HelpSolid} hovered={hover === 'help'} size="nav" />
            <span className="nav-icon-label">帮助</span>
          </button>

          {isMobile ? (
            <div className="top-nav-more-wrap">
              <button className="ui-btn nav-icon-btn" onClick={() => { setUserMenuOpen(false); setMoreOpen(v => !v); }}
                onMouseEnter={() => setHover('more')} onMouseLeave={() => setHover(null)} title="更多">
                <NavIcon outline={EllipsisHorizontalIcon} solid={EllipsisSolid} hovered={hover === 'more'} size="nav" />
                <span className="nav-icon-label">更多</span>
              </button>
              {moreOpen && (
                <div className="top-nav-dropdown" onMouseLeave={() => setMoreOpen(false)}>
                  <button type="button" className="top-nav-dropdown-item" onClick={() => { openModal('rank'); closeMenus(); }}>
                    <NavIcon outline={TrophyIcon} solid={TrophySolid} size="nav" /> 排行榜
                  </button>
                  <button type="button" className="top-nav-dropdown-item" onClick={() => { openModal('settings'); closeMenus(); }}>
                    <NavIcon outline={Cog6ToothIcon} solid={CogSolid} size="nav" /> 设置
                  </button>
                  {account && (
                    <button type="button" className="top-nav-dropdown-item" onClick={() => { closeMenus(); void logout(); }}>
                      {account.display_name || account.username} · 退出
                    </button>
                  )}
                </div>
              )}
            </div>
          ) : (
            <div className="top-nav-user-wrap">
              <button className="ui-btn nav-icon-btn" onClick={() => { setMoreOpen(false); setUserMenuOpen(v => !v); }}
                title={account ? `${account.display_name || account.username} · 账户菜单` : '账户菜单'}
                style={{ maxWidth: 120, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                {account ? (account.display_name || account.username) : '账户'}
              </button>
              {userMenuOpen && (
                <div className="top-nav-dropdown" onMouseLeave={() => setUserMenuOpen(false)}>
                  <button type="button" className="top-nav-dropdown-item" onClick={() => { openModal('rank'); closeMenus(); }}>
                    <NavIcon outline={TrophyIcon} solid={TrophySolid} size="nav" /> 排行榜
                  </button>
                  <button type="button" className="top-nav-dropdown-item" onClick={() => { openModal('settings'); closeMenus(); }}>
                    <NavIcon outline={Cog6ToothIcon} solid={CogSolid} size="nav" /> 设置
                  </button>
                  {account && (
                    <button type="button" className="top-nav-dropdown-item" onClick={() => { closeMenus(); void logout(); }}>
                      退出登录
                    </button>
                  )}
                </div>
              )}
            </div>
          )}
        </div>
      </header>
      {showCoach && <CoachMarks onDone={() => setShowCoach(false)} />}
    </>
  );
}
